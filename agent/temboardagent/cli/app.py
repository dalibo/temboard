...
class TemboardAgentApplication(BaseApplication):
    ...
    DEFAULT_PLUGINS_EP = "temboardagent.plugins"
    DEFAULT_PLUGINS = [
        "activity",
        "administration",
        "dashboard",
    ...
    def apply_config(self):
        ...
        self.postgres.connection_lost_observers.append(self.discover.connection_lost)
    ...
    def init_specs(self, app_specs):
        ...
        s = "postgresql"
        add_specs(
            OptionSpec(s, "host", default="/var/run/postgresql", validator=v.dir_),
    ...
    def log_versions(self):
        ...
        logger.debug("Using Python %s (%s).", versions["python"], versions["pythonbin"])
        logger.debug(
            "Using libpq %s, Psycopg2 %s.", versions["libpq"], versions["psycopg2"]
        )
...
def list_options_specs():
    ...
    yield OptionSpec(section, "address", default="0.0.0.0", validator=v.address)
    yield OptionSpec(section, "port", validator=v.port, default=2345)
    yield OptionSpec(
        section, "ssl_cert_file", default=OptionSpec.REQUIRED, validator=v.file_
    )
    yield OptionSpec(
        section, "ssl_key_file", default=OptionSpec.REQUIRED, validator=v.file_
    )
    yield OptionSpec(section, "ssl_ca_cert_file", validator=v.file_)
    yield OptionSpec(
        section, "signing_public_key", default="signing-public.pem", validator=v.path
    )

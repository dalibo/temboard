...
class Register(SubCommand):
    ...
    def main(self, args):
        ...
        try:
            ...
            logger.info("Instance registered. Manage it at %s.", dashboard_url)
        except TemboardClient.Error as e:
            raise UserError("UI returned error: %s" % e)
...
def ask_password():
    ...
    try:
        password = raw_pass
        validate_parameters({"password": password}, [("password", T_PASSWORD, False)])
    except HTTPError:
...
def ask_username():
    ...
    try:
        ...
        validate_parameters({"username": username}, [("username", T_USERNAME, False)])
    except HTTPError:

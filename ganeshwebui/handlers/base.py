...
from ganeshwebui.model.orm import Roles
from ganeshwebui.application import get_role_by_cookie
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def check_admin(self,):
        if isinstance(self.current_user, Roles) and \
            ...
            return
        raise GaneshError(401, "Unauthorized access.")

    def load_auth_cookie(self,):
        """
        Try to load secure cookie content.
        """
        cookie_content = self.get_secure_cookie('ganesh')
        if cookie_content is None:
            raise GaneshError(302, "Authentication cookie is missing.")
    ...
    def start_db_session(self):
        ...
        try:
            ...
            self.logger.error(e.message)
            raise GaneshError(500, "Unable to create a new database session.")

    ...
    def post_role(self, username):
        try:
            ...
            if 'new_username' not in data or data['new_username'] == '':
                raise GaneshError(400, "Username is missing.")
            if 'email' not in data or data['email'] == '':
                raise GaneshError(400, "Email is missing.")
            if 'groups' not in data:
                raise GaneshError(400, "Groups field is missing.")
            if 'is_active' not in data:
                raise GaneshError(400, "Active field is missing.")
            if 'is_admin' not in data:
                raise GaneshError(400, "Administrator field is missing.")

            if role and role.role_name != data['new_username']:
                if 'password' not in data or data['password'] == '':
                    raise GaneshError(400, "Username will be changed, you need to change the password too.")
            if role is None:
                if 'password' not in data or data['password'] == '':
                    raise GaneshError(400, "Password is missing.")
            if ('password' in data and data['password'] != '') and ('password2' not in data or data['password2'] == ''):
                raise GaneshError(400, "Password confirmation is missing.")
            if 'password' in data and 'password2' in data:
                if data['password'] != data['password2']:
                    raise GaneshError(400, "Password confirmation can not be checked.")
            ...
            if data['password']:
                ...
                h_passwd = hash_password(data['new_username'], data['password'])
            else:
                h_passwd = None
            ...
            if role:
                ...
                role_groups = get_groups_by_role(self.db_session, role.role_name)
                if role_groups:
                    for role_group in role_groups:
                        delete_role_from_group(self.db_session, role.role_name, role_group.group_name)
                role = update_role(self.db_session,
                            role.role_name,
                            data['new_username'],
                            h_passwd,
                            data['email'],
    ...
    def delete_role(self):
        try:
            ...
            if 'username' not in data or data['username'] == '':
                raise GaneshError(400, "Username field is missing.")

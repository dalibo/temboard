...
def get_routes(config):
    ...
    routes = [
        (r"/server/(.*)/([0-9]{1,5})/activity/running", ActivityHandler, handler_conf),
        (r"/server/(.*)/([0-9]{1,5})/activity/(blocking|waiting)$", ActivityWBHandler, handler_conf),
        (r"/proxy/(.*)/([0-9]{1,5})/activity", ActivityProxyHandler, handler_conf),
        (r"/proxy/(.*)/([0-9]{1,5})/activity/(blocking|waiting)$", ActivityWBProxyHandler, handler_conf),
...
class ActivityWBHandler(BaseHandler):
    def get_activity_w_b(self, agent_address, agent_port, mode):
        try:
            ...
            # Load activity.
            if mode == 'waiting':
                activity_data = ganeshd_activity_waiting(self.ssl_ca_cert_file, instance.agent_address, instance.agent_port, xsession)
            elif mode == 'blocking':
                activity_data = ganeshd_activity_blocking(self.ssl_ca_cert_file, instance.agent_address, instance.agent_port, xsession)
            else:
                raise GaneshError(404, "Mode unknown.")
            return HTMLAsyncResult(
                    http_code = 200,
                    template_path = self.template_path,
    ...
    def get(self, agent_address, agent_port, mode):
        run_background(self.get_activity_w_b, self.async_callback, (agent_address, agent_port, mode))
...
class ActivityWBProxyHandler(JsonHandler):
    def get_activity_w_b(self, agent_address, agent_port, mode):
        try:
            ...
            # Load activity.
            if mode == 'waiting':
                data_activity = ganeshd_activity_waiting(self.ssl_ca_cert_file, instance.agent_address, instance.agent_port, xsession)
            elif mode == 'blocking':
                data_activity = ganeshd_activity_blocking(self.ssl_ca_cert_file, instance.agent_address, instance.agent_port, xsession)
            else:
                raise GaneshError(404, "Mode unknown.")
            return JSONAsyncResult(http_code = 200, data = data_activity)
        except (GaneshError, GaneshdError, Exception) as e:

...
def get_loadaverage(session, hostname, start, end):
    ...
    query = "COPY (SELECT to_char(datetime, 'YYYY/MM/DD HH24:MI:SS') AS date, load1, load5, load15 FROM supervision.%s WHERE hostname = '%s' AND datetime >= '%s' AND datetime <= '%s' ORDER BY 1) TO STDOUT WITH CSV HEADER"
...
def get_memory(session, hostname, start, end):
    ...
    query = "COPY (SELECT to_char(datetime, 'YYYY/MM/DD HH24:MI:SS') AS date, mem_free AS free, mem_cached AS cached, mem_buffers AS buffers, (mem_used - mem_cached - mem_buffers) AS other  FROM supervision.%s WHERE hostname = '%s' AND datetime >= '%s' AND datetime <= '%s' ORDER BY 1) TO STDOUT WITH CSV HEADER"
...
def get_swap(session, hostname, start, end):
    ...
    query = "COPY (SELECT to_char(datetime, 'YYYY/MM/DD HH24:MI:SS') AS date, swap_used AS used FROM supervision.%s WHERE hostname = '%s' AND datetime >= '%s' AND datetime <= '%s' ORDER BY 1) TO STDOUT WITH CSV HEADER"
...
def get_hitreadratio(session, hostname, port, start, end):
    ...
    query = "COPY (SELECT to_char(datetime, 'YYYY/MM/DD HH24:MI:SS') AS date, ROUND((SUM(blks_hit)::FLOAT/(SUM(blks_hit) + SUM(blks_read)::FLOAT) * 100)::numeric, 2) AS hit_read_ratio FROM supervision.%s WHERE hostname = '%s' AND port = %s AND datetime >= '%s' AND datetime <= '%s' GROUP BY 1, hostname, port ORDER BY 1) TO STDOUT WITH CSV HEADER"
...
def get_written_buffers(session, hostname, port, start, end):
    ...
    query = "COPY (SELECT to_char(datetime, 'YYYY/MM/DD HH24:MI:SS') AS date, buffers_checkpoint AS checkpoint, buffers_clean AS clean, buffers_backend AS backend FROM supervision.%s WHERE hostname = '%s' AND port = %s AND datetime >= '%s' AND datetime <= '%s' ORDER BY 1) TO STDOUT WITH CSV HEADER"

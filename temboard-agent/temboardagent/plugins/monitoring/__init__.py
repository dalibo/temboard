...
from temboardagent.configuration import (PluginConfiguration,
                                         ConfigurationError)
...
from monitoring.probes import (probe_sessions, probe_xacts, probe_locks,
                               probe_blocks, probe_bgwriter, probe_db_size,
                               probe_tblspc_size, probe_filesystems_size,
                               probe_cpu, probe_process, probe_memory,
                               probe_loadavg, probe_wal_files,
                               probe_replication, load_probes, run_probes)
...
def monitoring_probe_filesystems_size(http_context, queue_in=None, config=None,
    ...
    try:
        output = api_run_probe(probe_filesystems_size(
                        config.plugins['monitoring']),
                    config)
        return output
    except (Exception, error) as e:
        logger.error(str(e.message))
    ...
    class Configuration(PluginConfiguration):
        def __init__(self, config, *args, **kwargs):
            ...
            try:
                ...
                raise ConfigurationError("SSL CA certificates file %s can't be"
                                         " opened." % (
                                             self.get(__name__,
                                                      'ssl_ca_cert_file')))
            except NoOptionError:

...
category, name
    ...
    for row in conn.get_rows():
        ...
        if row['name'] not in do_not_format_names and row['vartype'] == u'integer':
            ...
            row_dict['boot_val'] = int(human_to_number(row_dict['boot_val'], row_dict['unit']))
            if in_auto_conf:
                row_dict['auto_val'] = int(human_to_number(row_dict['auto_val'], row_dict['unit']))
            if in_file_conf:
                row_dict['file_val'] = int(human_to_number(row_dict['file_val'], row_dict['unit']))
        elif row['name'] not in do_not_format_names and row['vartype'] == u'real':
...
def get_settings_status(conn, config, http_context):
    ...
    return {'restart_pending': pending_restart,
                ...
                for pg_config_item in pg_config_category['rows']:
                    if pg_config_item['name'] == setting['name']:
                        ...
                        if pg_config_item['vartype'] == u'bool':
                            # Boolean handling.
                            if setting['setting'].lower() not in [u'on', u'off']:
                                raise HTTPError(406, 'Invalid setting: %s.' % (setting['setting'].lower()))
                            checked = True
...
def post_hba_raw(conn, config, http_context):
    ...
    logger.debug(http_context['post'])
...
def post_hba(conn, config, http_context):
    ...
    for entry in http_context['post']['entries']:
        if 'comment' in entry and len(entry['comment']) > 0:
            ...
            try:
                new_hba_entry.connection = entry['connection'] if 'connection' in entry else ''
                new_hba_entry.database = entry['database'] if 'database' in entry else ''
                new_hba_entry.user = entry['user'] if 'user' in entry else ''
                new_hba_entry.address = entry['address'] if 'address' in entry else ''
                new_hba_entry.auth_method = entry['auth_method'] if 'auth_method' in entry else ''
                new_hba_entry.auth_options = entry['auth_options'] if 'auth_options' in entry else ''
            except Exception as e:
                logger.error(e.message)
...
def get_pg_ident(conn, config, http_context):
    ...
    try:
        conn.execute("SELECT setting FROM pg_settings WHERE name = 'ident_file'")

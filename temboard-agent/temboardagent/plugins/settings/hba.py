...
class HBAManager(ConfigurationFileManager):
    ...
    def options(self, conn):
        ...
        conn.execute("SELECT rolname FROM pg_user JOIN pg_auth_members ON (pg_user.usesysid=pg_auth_members.member) JOIN pg_roles ON (pg_roles.oid=pg_auth_members.roleid) ORDER BY rolname ASC")
        for row in conn.get_rows():
            users.append("+%s" % (row['rolname']))
        # Get databases name
        conn.execute("SELECT datname FROM pg_database WHERE NOT datistemplate ORDER BY datname ASC")
        for row in conn.get_rows():
...
class HBAEntry:
    ...
    def lazy_check(self):
        ...
        if self.connection != 'local' and \
            len(self.address) == 0:
            raise HTTPError(406, "An address is required for method '%s'." % (self.connection))
    ...
    def parse(cls, raw):
        ...
        if not raw.startswith('#') and len(raw) > 0:
            ...
            r_host_address = re.compile(r'^' + p_connection_host + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_address + r'\s+'\
                                            + p_anything \
                                            + p_anything_end)
            r_host_ip_address = re.compile(r'^' + p_connection_host + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_ip_address + r'\s+'\
                                            + p_ip_mask + r'\s+'\
                                            + p_anything \
                                            + p_anything_end)
            r_local = re.compile(r'^' + p_connection_local + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_anything + r'\s+'\
                                            + p_anything \
                                            + p_anything_end)

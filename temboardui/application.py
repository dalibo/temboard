...
def add_role(session, role_name, role_password, role_email, is_active = True, is_admin = False):
    try:
        ...
        if e.message.find('roles_role_email_key') > 0:
            raise TemboardUIError(400, "Email address '%s' already in use." % (role_email))
...
def add_group(session, group_name, group_description, group_kind):
    try:
        ...
        if e.message.find('groups_group_kind_check') > 0:
            ...
            raise TemboardUIError(400, "Group '%s' ('%s') already exists." % (group_name, group_kind))
        else:
...
def get_group(session, group_name, group_kind):
    try:
        ...
        raise TemboardUIError(400, "Group '%s' (%s) not found." % (group_name, group_kind))
    except Exception as e:
        raise TemboardUIError(e.message)
...
def update_group(session, group_name, group_kind, new_group_name = None, group_description = None):
    try:
        ...
        if e.message.find('groups_pkey') > 0:
            raise TemboardUIError(400, "Group name '%s' already in use." % (new_group_name))
        else:
            raise TemboardUIError(400, e.message)
    except AttributeError as e:
        raise TemboardUIError(400, "Group '%s' ('%s') not found." % (group_name, group_kind))
...
def add_role_in_group(session, role_name, group_name):
    try:
        ...
        if e.message.find('role_groups_group_name_fkey') > 0:
            raise TemboardUIError(400, "Group '%s' ('role') does not exist." % (group_name))
            ...
            raise TemboardUIError(400, "Role '%s' already in group '%s'." % (role_name, group_name))
        else:
            raise TemboardUIError(400, e.message)
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def delete_role_from_group(session, role_name, group_name):
    try:
        ...
        raise TemboardUIError(400, "Role '%s' not found in group '%s'." % (role_name, group_name))
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def add_instance(session,
    ...
    try:
        ...
        if e.message.find('instances_pkey') > 0:
            raise TemboardUIError(400, "Instance entry ('%s:%s') already exists." % (agent_address, agent_port))
        else:
            raise TemboardUIError(400, e.message)
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def get_instance(session, agent_address, agent_port):
    try:
        ...
        raise TemboardUIError(400, "Instance entry '%s:%s' not found." % (agent_address, agent_port))
    except Exception as e:
        raise TemboardUIError(e.message)
...
def add_instance_in_group(session, agent_address, agent_port, group_name):
    try:
        ...
        if e.message.find('instance_groups_group_name_fkey') > 0:
            raise TemboardUIError(400, "Group '%s' ('instance') does not exist." % (group_name))
        elif e.message.find('instance_groups_agent_address_fkey') > 0:
            raise TemboardUIError(400, "Instance entry ('%s:%s') does not exist." % (agent_address, agent_port))
        elif e.message.find('instance_groups_pkey') > 0:
            raise TemboardUIError(400, "Instance entry ('%s:%s)' already in group '%s'." % (agent_address, agent_port, group_name))
        else:
            raise TemboardUIError(400, e.message)
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def add_instance_plugin(session, agent_address, agent_port, plugin_name):
    try:
        ...
        if e.message.find('plugins_pkey') > 0:
            raise TemboardUIError(400, "Plugin '%s' was already activated for this instance." % (plugin_name))
        elif e.message.find('plugins_agent_address_fkey') > 0:
            raise TemboardUIError(400, "Instance '%s:%s' does not exist." % (agent_address, agent_port))
        else:
            raise TemboardUIError(400, e.message)
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def delete_instance_from_group(session, agent_address, agent_port, group_name):
    try:
        ...
        raise TemboardUIError(400, "Instance entry ('%s:%s)' not found in group '%s'." % (agent_address, agent_port, group_name))
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def add_role_group_in_instance_group(session, role_group_name, instance_group_name):
    try:
        ...
        if e.message.find('access_role_instance_pkey') > 0:
            raise TemboardUIError(400, "Group '%s' ('role') has already access to '%s'." % (role_group_name, instance_group_name))
        elif e.message.find('access_role_instance_instance_group_name_fkey') > 0:
            raise TemboardUIError(400, "Instance group '%s' does not exist." % (instance_group_name))
        elif e.message.find('access_role_instance_role_group_name_fkey') > 0:
            raise TemboardUIError(400, "Role group '%s' does not exist." % (role_group_name))
        else:
            raise TemboardUIError(400, e.message)
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def delete_role_group_from_instance_group(session, role_group_name, instance_group_name):
    try:
        ...
        raise TemboardUIError(400, "Role group '%s' not found in instance group '%s'." % (role_role_name, instance_group_name))
    except Exception as e:
        raise TemboardUIError(400, e.message)
...
def get_role_by_auth(session, role_name, role_password):
    try:
        ...
        if role.role_password != unicode(role_password):
            raise TemboardUIError(400, "Wrong user/password: %s/%s" % (role_name, role_password))
        return role
    except NoResultFound as e:
        raise TemboardUIError(400, "Wrong user/password: %s/%s" % (role_name, role_password))
    except Exception as e:
...
def check_role_name(role_name):
    ...
    if not r_role_name.match(role_name):
        raise TemboardUIError(400, "Invalid username, must satisfy this regexp pattern: %s" % (p_role_name))
...
def check_group_name(group_name):
    ...
    if not r_group_name.match(group_name):
        raise TemboardUIError(400, "Invalid group name, must satisfy this regexp pattern: %s" % (p_group_name))

def check_group_description(group_description):
    if len(group_description) > 255:
        raise TemboardUIError(400, "Invalid group description, must be a 256 char (max) length string.")

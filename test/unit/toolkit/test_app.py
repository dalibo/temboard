...
from temboardagent.toolkit.app import BaseApplication
...
def test_user_error():
    from temboardagent.toolkit.errors import UserError
...
def test_unhandled_error_debug(mocker):
    pm = mocker.patch('temboardagent.toolkit.app.pdb.post_mortem')
...
def test_apply_config_bad_logging(mocker):
    ...
    from temboardagent.toolkit.app import BaseApplication, UserError
...
def test_read_file(mocker):
    ...
    app = BaseApplication()
    open_ = mocker.patch('temboardagent.toolkit.app.open', create=True)
...
def test_reload(mocker):
    m = 'temboardagent.toolkit.app.BaseApplication'
...
def test_fetch_plugin(mocker):
    iter_ep = mocker.patch(
        'temboardagent.toolkit.app.pkg_resources.iter_entry_points')
    from temboardagent.toolkit.app import BaseApplication
...
def test_purge_plugins():
    from temboardagent.toolkit.app import BaseApplication, MergedConfiguration
...
def test_debug_arg():
    ...
    from temboardagent.toolkit.app import define_core_arguments
...
def test_debug_var():
    from temboardagent.toolkit.app import detect_debug_mode

...
def test_resolve_main(mocker):
    from temboardagent.toolkit.proctitle import compute_main_module_name
...
def test_fix_argv(mocker):
    cmmn = mocker.patch(
        'temboardagent.toolkit.proctitle.compute_main_module_name',
        autospec=True)
    from temboardagent.toolkit.proctitle import PY3, fix_argv
...
def test_read_memory():
    import ctypes
    from temboardagent.toolkit.proctitle import PY3, read_byte
...
def test_walk_bytes_backwards():
    import ctypes
    from temboardagent.toolkit.proctitle import reverse_walk_memory
...
def test_find_nulstrings():
    from temboardagent.toolkit.proctitle import reverse_find_nulstring
...
def test_find_stack_segment():
    from temboardagent.toolkit.proctitle import find_stack_segment_from_maps
...
def test_find_argv_from_procmaps(mocker):
    mod = 'temboardagent.toolkit.proctitle'
    ...
    from temboardagent.toolkit.proctitle import find_argv_memory_from_maps
...
def test_set_proc_title(mocker):
    memmove = mocker.patch(
        'temboardagent.toolkit.proctitle.ctypes.memmove', autospec=True)
    from temboardagent.toolkit.proctitle import ProcTitleManager

...
ADD COLUMN dba_group_id INTEGER REFERENCES application.groups(id);
...
CREATE TABLE application.memberships (
	role_name TEXT REFERENCES application.roles(role_name) NOT NULL,
	group_id INTEGER REFERENCES application.groups(id) NOT NULL,
	cdate TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
...
JOIN application.memberships AS m
...
SELECT DISTINCT
	e.name || '/dba' AS name,
	'Group for ' || e.name || ' DBAs' AS description
...
WITH legacy AS (
	...
	SELECT DISTINCT
	  	rxg.role_name,
		string_agg(DISTINCT ixg.group_name, '-' ORDER BY ixg.group_name) || '/dba' AS group_name
	-- instance_groups has no entry for empty groups.
	...
	GROUP BY 1, ixg.agent_address, ixg.agent_port
)
INSERT INTO application.memberships (role_name, group_id)
SELECT DISTINCT r.role_name, g.id
FROM legacy AS r
JOIN application.groups AS g
  ON g.name = r.group_name

UNION

-- Create a membership for each empty group.
SELECT DISTINCT
	rxg.role_name,
	e.dba_group_id

...
_REQUIRED_POST_HEADERS = _REQUIRED_HEADERS | {"content-type", "content-length"}
...
def canonicalize_request(method, path, headers, body=b""):
    ...
    if missing_headers:
        ...
        raise TemboardError("Missing headers for request signing: %s" % missing_headers)
    ...
    lines = [
        ...
        [
            ...
            for name, value in canonicalized_headers.items()
        ]
    )
    ...
    if b"POST" == method:
        h = sha256()
        h.update(body)
        lines += [b"", h.hexdigest().encode("ascii")]
...
def load_public_key(data):
    return serialization.load_pem_public_key(data, backend=openssl_backend)


HASH = hashes.SHA256()
PADDING = padding.PSS(mgf=padding.MGF1(HASH), salt_length=padding.PSS.MAX_LENGTH)

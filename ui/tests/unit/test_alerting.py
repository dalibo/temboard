def test_preproc():
    ...
    data = dict(
        load1=dict(data={"loadavg": [{"load1": 1}]}, expected=1.0),
        cpu_core=dict(
            data={
                "cpu": [
                    {
        ...
        memory_usage=dict(
            data={"memory": [{"mem_total": 10, "mem_free": 2, "mem_cached": 3}]},
            expected=50.0,
        ),
        swap_usage=dict(
            data={"memory": [{"swap_total": 10, "swap_used": 5}]}, expected=50.0
        ),
        fs_usage_mountpoint=dict(
            data={"filesystems_size": [{"mount_point": ".", "total": 10, "used": 5}]},
            expected={".": 50.0},
        ),
        wal_files_archive=dict(
            data={"wal_files": [{"archive_ready": 10}]}, expected=10
        ),
        wal_files_total=dict(data={"wal_files": [{"total": 10}]}, expected=10),
        rollback_db=dict(
        ...
        waiting_sessions_db=dict(
            data={"sessions": [{"dbname": "toto", "waiting": 1}]}, expected={"toto": 1}
        ),
        replication_lag=dict(data={"replication_lag": [{"lag": 123}]}, expected=123),
        replication_connection=dict(
            data={"replication_connection": [{"upstream": "foo", "connected": 1}]},
            expected={"foo": 1},
        ),
        temp_files_size_delta=dict(
            data={"temp_files_size_delta": [{"dbname": "foo", "size": 123}]},
            expected={"foo": 123},
        ),
        heap_bloat=dict(

...
def test_redirect_response_host():
    ...
    response = protocol_switcher(Mock(headers={"Host": "temboard.lan"}, uri="/home"))
...
def test_easy_handshake_ok(ioloop_mock, mocker):
    ...
    from temboardui.autossl import EasySSLIOStream, ssl
...
def test_easy_handshake_other_errors(ioloop_mock, mocker):
    mocker.patch("temboardui.autossl.EasySSLIOStream._add_io_state")
    from temboardui.autossl import EasySSLIOStream, errno, socket, ssl
...
def test_handle_connection(mocker):
    handle_stream = mocker.patch("temboardui.autossl.AutoHTTPSServer.handle_stream")
...
def test_handle_stream_ssl(mocker):
    parent_handle_stream = mocker.patch("temboardui.autossl.HTTPServer.handle_stream")
...
def test_handle_stream_closed(mocker):
    mocker.patch("temboardui.autossl.AutoHTTPSServer.handle_http_connection")
